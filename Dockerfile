...
RUN pnpm install --filter "@packing-list/api..." --frozen-lockfile --unsafe-perm
RUN pnpm --filter "@packing-list/api" run build
RUN pnpm --filter "@packing-list/api" run gen
